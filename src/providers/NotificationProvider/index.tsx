...
const NotificationContext = React.createContext<Notificationtate | undefined>(
  DefaultValue
);
...
const NotificationProvider = ({ children }: NotificationProviderProps) => {
  ...
  React.useEffect(() => {
    const timetout = setTimeout(
      () => setNotificaiton(DefaultValue.state),
      1500
    );
  ...
  return (
    <NotificationContext.Provider
      value={{ state: notification, dispatch: (val) => setNotificaiton(val) }}
    >
      {notification ? (
        <NotificaitonBox
          ...
          img={
            notification.status === "success"
              ? getCloudinaryUrl("assets/check_vc1lwo.png")
              : getCloudinaryUrl("assets/error_leeg7e.png")
          }
        />
      ) : null}
      {children}
...
const useNotificationProvider = () => {
  ...
  if (context === undefined) {
    throw new Error(
      'useNotificationProvider must be used within a ThemeProvider'
    );
  }
    ...
    {display ? (
      ...
      >
        <div
          ...
          className="flex items-center px-4 rounded-lg h-9 w-60 bg-gray500 dark:bg-white text-black lg:h-11"
        >
          <Image
            data-testid="notificaiton-box-image"
            src={img}
            width={18}
            height={18}
            alt="Notification"
          />
          <p
            data-testid="notificaiton-box-note"
            className="pl-2 text-sm lg:pl-4"
          >
            {note}
          </p>
        </div>
